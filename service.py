...
try:
 ...
 p=TracerProvider(resource=Resource.create({"service.name":"ai-data-governance-platform"}));p.add_span_processor(BatchSpanProcessor(ConsoleSpanExporter()));trace.set_tracer_provider(p)
...
def handle(r:Request):
 with tracer.start_as_current_span("ai-data-governance-platform.domain"):
  try: a=Asset(r.key,r.payload.get("owner",""),r.payload.get("classification","internal"),set(r.payload.get("tags",[])));d=evaluate(a,r.payload.get("required_classification","internal"),r.payload.get("actor",""));return {"allowed":d.allowed,"reasons":d.reasons}
